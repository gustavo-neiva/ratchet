...
- selftest.sh: PATH pollution between suites — when a test exports `PATH="$tmpdir/bin:$PATH"` and doesn't restore it, stale stub paths leak into later tests. Suites that modify PATH must save it at start and restore at end. Symptom: "No such file or directory" for deleted tmpdir/bin/git.
