...
- [IN PROGRESS] T1.1 (normal, serial) `lib/tracker.sh`: add `tracker_next_tag` — echo the tag (`trivial|normal|hard`) of the first `[IN PROGRESS]` task, else the first `[ ]` task; echo `normal` when untagged or no task. Pure addition; do not modify existing functions. Add selftest cases: tagged trivial, tagged hard, untagged, empty tracker.
