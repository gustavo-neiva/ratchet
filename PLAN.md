...
- [IN PROGRESS] T1.5 (normal, serial) `ratchet doctor`: print the three effective tier chains and thinking levels (or "→ MODELS (flat)" when unset). Warn (not fail) when `LIGHT_MODELS` is set but `THINKING_LIGHT` is not `off` (cheap tier should not reason). Add selftest for the warning.
