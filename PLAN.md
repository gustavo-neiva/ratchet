...
- [IN PROGRESS] T1.2 (normal, serial) `lib/contract.sh` + `lib/common.sh`: add the six keys `PLAN_MODELS BUILD_MODELS LIGHT_MODELS THINKING_PLAN THINKING_BUILD THINKING_LIGHT` to `CONTRACT_KEYS` and as empty-string defaults in common.sh (commented like the neighbors). Add selftest: a conf file with `LIGHT_MODELS="a/b"` parses; an unknown key still errors.
